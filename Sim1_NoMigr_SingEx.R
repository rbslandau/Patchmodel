...
for (N in f_mort){
  for (E in f_emerg){
    for (G in f_growth){
      ...
      adult_exp_151_190 <- new("odeModel", 
                          main = function (time, init, parms) {
                            with(as.list(c(init, parms)), {
                              # Patch I (polluted)
                              dNi <-   -r1 * Ni + 
      ...
      # Call ODE model  
      res_earlinst191_360 <- sim(earlinst_exp_191_360)
...
for(I in 1:length(levels(d$treat)))
     {
     	 png(filename = file.path(getwd(), paste0("figures/", "Sim1_SingleEvent_NoMigr", levels(d$treat)[I], ".png"))) 
...
write.csv(final, file= paste0(getwd(),"/Sim1_NoMigr_SingEx_rec_time_new.csv"), row.names=F)
 
# Save recovery times
write.csv(d, file = paste0(getwd(),"/Z_Sim1_NoMigr_SingEx_daily_Ns.csv"))
