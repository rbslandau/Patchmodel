## ---------------------------- #
# Simulation 2.1 for scenario
# ---------------------------- #
# Pesticide event: multiple exposure, on day 120
# symmetric migration from an upstream patch
...
for (N in f_mort){
  for (E in f_emerg){
    for (G in f_growth){
      ...
      # ---------------------------------------------------------------------- #
      # Multiple exposure on day day_pesticide in each year of the simulation:  
      # ---------------------------------------------------------------------- #
...
for(I in 1:length(levels(d$treat)))
     {
     	 png(filename = file.path(getwd(), paste0("figures/", "Sim4_MultipleEvent_symmMigr", levels(d$treat)[I], ".png"))) 
...
write.csv(final, file= paste0(getwd(),"/Sim4_symmMigr_MultiEx_rec_time_new.csv"), row.names=F)
 
# Save recovery times
write.csv(d, file = paste0(getwd(),"/Z_Sim4_symmMigr_MultiEx_daily_Ns.csv"))