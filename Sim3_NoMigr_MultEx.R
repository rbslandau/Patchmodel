# ---------------------------------------------- #
# Simulation 3 for scenario:
# ---------------------------------------------- #
# Pesticide events: multiple exposures, on day 120 of each year
# No migration from an upstream patch
...
diff_temp <- merge(diff_temp_a, v3)
  
# for comparison of different scenarios with different baseline recovery times
# calculation of difference to baseline scenario: 
...
for(I in 1:length(levels(d$treat)))
     {
     	 png(filename = file.path(getwd(), paste0("figures/", "Sim3_MultiEvent_NoMigr", levels(d$treat)[I], ".png"))) 
...
write.csv(final, file= paste0(getwd(),"/Sim3_NoMigr_MultEx_rec_time_new.csv"), row.names=F)
 
# Save recovery times
write.csv(d, file = paste0(getwd(),"/Z_Sim3_NoMigr_MultEx_daily_Ns.csv"))
