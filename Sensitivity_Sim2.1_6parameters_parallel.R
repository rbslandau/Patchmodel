# ======================================================================================== #
# Sensitivity analysis of the model using variance based techniques for >>> 6 parameters <<<  #
...
#              - vary 6 main parameters, excluding the structural ones (l_eggs & l_adult) 
# Author: Bernhard Kuehn 
# last change: 20.02.2017
# revised by RBS (05.03.2017)
# ==================================================================================================== #
...
# Simulation 2.1: symmetic Migration, single exposure
# Pesticide event: single, on day pesticide (120)
...
# Emigration rate: larvae 0.2, adult 0.3
# mortality rate of migration: 0.3 for larvae and 0.3 for adults
...
params = data.frame(# no theta logistic growth
                    eps_linst = 0.2, # migration rate larvae
...
simulate.model.sim2.1 = function(X){
  ...
  l_adult = 40 # duration of adult period
   ...
   # define growth-rate of eggs based on days eggs need to grow
    r_earlinst <- -(log((1/0.9-1)*(0.5*K)/(K-(0.5*K)))/(l_egg))
    ...
    N_daily[,1] <- 1:(sim_years*360)
          ...
          adult151_190 <- new("odeModel", 
                              ...
                              parms  = c(r1 = 0, K = K,d = X[[iter,4]], eps = X[[iter,2]], 
                                         r2 = 0, s = X[[iter,5]], theta= X[[iter,1]]),
                              times  = c(from = 151, to = 150+l_adult, by = 1),
          ...
          earlinst191_360 <- new("odeModel", 
                             ...
                             times  = c(from = 150+l_adult, to = 360, by = 1),
...
# theta, eps_linst, eps_adult, d_linst, d_adult, s, day_pest
# +-50%
# parameters constant: K, rd_linst. 
...
# simulate and write execution time into console
ptm <- proc.time()
sens.out.S2.1.noStructuralChanges = tryCatch(
  soboljansen(model = simulate.model.sim2.1,X1,X2,nboot = 1000),
  error = function(e) print(e)
)
proc.time() - ptm

# save output
save(sens.out.S2.1.noStructuralChanges,file = "../Sens.out.S2.1.noStructuralChanges.RData")
