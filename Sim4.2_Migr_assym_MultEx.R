# ---------------------------- #
# Simulation 4.2 for scenario
# ---------------------------- #
# Pesticide event: multiple exposure, on day_pesticide (i.e. 120) of each year
...
for (N in f_mort){
  for (E in f_emerg){
    for (G in f_growth){
      ...
      # -------------------------------------------------------------------- #
      # Multiple exposure at day day_pesticide of each year of the simulation:  
      # -------------------------------------------------------------------- #
      ...
      linst1_x <- new("odeModel", 
                      ...
                      parms  = c(r1 = r_linst, K = K,d = d_linst, eps_up = eps_linst_up,
      ...
      N_daily[i[j]:(i-1)[j+1],2:3] <- rbind(res_linst1_x_mat[,-1],
...
for(I in 1:length(levels(d$treat)))
     {
     	 png(filename = file.path(getwd(), paste0("figures/", "Sim4_MultiEvent_asymmMigr", levels(d$treat)[I], ".png"))) 
      ...
      legend(20,5900, c("Nj"), lty=1, lwd=2)    
        ...
        paste0(eps_linst_up," / ",eps_linst_down),paste0(eps_adult_up," / ",eps_adult_down), unique(d[d$treat == levels(d$treat)[I],"f_mort"]), unique(d[d$treat == levels(d$treat)[I],"f_emerg"]), unique(d[d$treat == levels(d$treat)[I],"f_growth"])), cex = 0.9)
      text(c(1700,1700,2800,2800,2800), c(6980,5900,6980,6480,5900), c("eps_linst = ", "eps_adult = ","f_mort =","f_emerg =","f_growth ="), cex = 0.9)
   dev.off()                                                                    
}

# Export daily population size
write.csv(final, file= paste0(getwd(),"/Sim4_asymmMigr_MultiEx_rec_time_new.csv"), row.names=F)
 
# Save recovery times
write.csv(d, file = paste0(getwd(),"/Z_Sim4_asymmMigr_MultiEx_daily_Ns.csv"))